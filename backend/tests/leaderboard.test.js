...
const registerAndLogin = async (role, suffix) => {
  ...
  if (reg.status !== 201) {
    throw new Error(`Registration failed for role ${role}: ${JSON.stringify(reg.body)}`);
  }
...
beforeEach(async () => {
  ...
  const studentUser = await User.findOne({ email: 'studentlb1@leaderboard.test' });
...
describe('Leaderboard API', () => {
  ...
  it('PATCH /api/leaderboard/handles — student can update handles', async () => {
    const res = await request(server)
      ...
      .send({ leetcode: 'testuser_lc', codeforces: 'testuser_cf', github: 'testuser_gh' });
  ...
  it('GET /api/leaderboard — returns sorted leaderboard entries', async () => {
    ...
    await LeaderboardEntry.create({
      ...
      userId: userA._id,
      platform: {
        leetcode: { solved: 200, ranking: 50000, easyCount: 80, mediumCount: 90, hardCount: 30 },
        codeforces: { rating: 1500, maxRating: 1600, rank: 'specialist', maxRank: 'expert' },
    ...
    await LeaderboardEntry.create({
      ...
      userId: userB._id,
      platform: {
        leetcode: { solved: 50, ranking: 200000, easyCount: 30, mediumCount: 15, hardCount: 5 },
        codeforces: { rating: 800, maxRating: 850, rank: 'newbie', maxRank: 'newbie' },
    ...
    expect(res.body.data.entries[0].totalScore).toBeGreaterThan(res.body.data.entries[1].totalScore);
  });
