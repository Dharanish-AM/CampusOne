...
let getEmbeddingSpy, chatSpy, initCollectionSpy, upsertPointsSpy, getPointCountSpy, searchSpy;
...
const registerAndLoginStudent = async (email, rollNumber) => {
  ...
  if (reg.status !== 201) {
    throw new Error(`Registration failed in test helper: ${JSON.stringify(reg.body)}`);
  }
...
beforeEach(async () => {
  ...
  getEmbeddingSpy = jest.spyOn(ollamaService, 'getEmbedding').mockResolvedValue(new Array(768).fill(0.123));
  chatSpy = jest.spyOn(ollamaService, 'chat').mockResolvedValue('Mocked response from Ollama');

  initCollectionSpy = jest.spyOn(qdrantService, 'initCollection').mockResolvedValue(undefined);
  upsertPointsSpy = jest.spyOn(qdrantService, 'upsertPoints').mockResolvedValue(undefined);
  getPointCountSpy = jest.spyOn(qdrantService, 'getPointCount').mockResolvedValue(0);
  searchSpy = jest.spyOn(qdrantService, 'search').mockResolvedValue([
    'Mocked FAQ: attendance condonation threshold is 65%.',
    'Mocked FAQ: semester registration rules apply.'
  ]);

  const credentials = await registerAndLoginStudent('chat_student@campus.edu', 'ROLL-CHAT-999');
...
describe('AI Chat API Test Suite', () => {
  ...
  it('should complete valid RAG flow and return reply', async () => {
    ...
    expect(getEmbeddingSpy).toHaveBeenCalledWith('What is the attendance policy?');
  ...
  it('should include conversation history in system prompt or message context', async () => {
    ...
    await ChatHistory.insertMany([
      { userId, conversationId, role: 'user', content: 'Hi there!', createdAt: new Date(Date.now() - 2000) },
      { userId, conversationId, role: 'assistant', content: 'Hello student, how can I help you today?', createdAt: new Date(Date.now() - 1000) },
    ]);
    ...
    expect(messagesSent[1].content).toBe('Hello student, how can I help you today?');
    expect(messagesSent[2].content).toBe('Tell me about the bus.');

    // DB should now have 4 turns for this conversation
    const historyCount = await ChatHistory.countDocuments({ userId, conversationId });
  ...
  it('should return chronological chat history', async () => {
    ...
    await ChatHistory.insertMany([
      { userId, conversationId, role: 'user', content: 'First message', createdAt: new Date(Date.now() - 2000) },
      { userId, conversationId, role: 'assistant', content: 'First reply', createdAt: new Date(Date.now() - 1000) },
    ]);
