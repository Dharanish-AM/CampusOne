...
const MEDAL_BG = ['rgba(255,215,0,0.15)', 'rgba(192,192,192,0.1)', 'rgba(205,127,50,0.1)'];

// ─── Helper to compute platform-specific sort score ──────────────────────────
const getPlatformScore = (entry, key) => {
  switch (key) {
    case 'leetcode': return entry.platform?.leetcode?.solved || 0;
    case 'codeforces': return entry.platform?.codeforces?.rating || 0;
    case 'github': return (entry.platform?.github?.totalStars || 0) * 5 + (entry.platform?.github?.publicRepos || 0) * 2;
    default: return entry.totalScore || 0;
  }
};
...
const PodiumCard = ({ entry, position, filterKey }) => {
  ...
  return (
    <View style={[styles.podiumCard, { backgroundColor: MEDAL_BG[position] }]}>
      ...
      <View style={[styles.podiumScoreBadge, { borderColor: MEDAL_COLORS[position] }]}>
        <Text style={[styles.podiumScore, { color: MEDAL_COLORS[position] }]}>{score.toLocaleString()}</Text>
      </View>
    </View>
  );
};
...
const LeaderboardRow = ({ entry, onPress, filterKey }) => {
  ...
  return (
    <TouchableOpacity style={styles.rowCard} onPress={() => onPress(entry)} activeOpacity={0.75}>
      <Text style={styles.rowRank}>#{entry.rank}</Text>
      <View style={styles.rowAvatar}>
        <Text style={styles.rowAvatarText}>{initials}</Text>
      </View>
      <View style={styles.rowInfo}>
        <Text style={styles.rowName} numberOfLines={1}>{entry.userId?.name || 'Student'}</Text>
        <Text style={styles.rowDept} numberOfLines={1}>{entry.studentId?.department || '—'}</Text>
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      ...
      <View style={styles.sheetContainer}>
        ...
        <Text style={styles.sheetDept}>{entry.studentId?.department} · {entry.studentId?.rollNumber}</Text>
        ...
        <View style={styles.sheetStats}>
          ...
          <View style={styles.sheetPlatform}>
            <View style={styles.sheetPlatformHeader}>
              ...
              <Text style={[styles.sheetPlatformTitle, { color: '#FFA116' }]}>LeetCode</Text>
            </View>
            <Text style={styles.sheetStat}>Solved: <Text style={styles.sheetStatValue}>{lc.solved || 0}</Text></Text>
            <Text style={styles.sheetStat}>Easy: <Text style={[styles.sheetStatValue, { color: '#4ADE80' }]}>{lc.easyCount || 0}</Text>  Med: <Text style={[styles.sheetStatValue, { color: '#FBBF24' }]}>{lc.mediumCount || 0}</Text>  Hard: <Text style={[styles.sheetStatValue, { color: '#F87171' }]}>{lc.hardCount || 0}</Text></Text>
            <Text style={styles.sheetStat}>Global Rank: <Text style={styles.sheetStatValue}>{(lc.ranking || 0).toLocaleString()}</Text></Text>
          </View>
          ...
          <View style={styles.sheetPlatform}>
            <View style={styles.sheetPlatformHeader}>
              ...
              <Text style={[styles.sheetPlatformTitle, { color: '#5CC2F2' }]}>Codeforces</Text>
            </View>
            <Text style={styles.sheetStat}>Rating: <Text style={styles.sheetStatValue}>{cf.rating || 0}</Text></Text>
            <Text style={styles.sheetStat}>Max Rating: <Text style={styles.sheetStatValue}>{cf.maxRating || 0}</Text></Text>
            <Text style={styles.sheetStat}>Rank: <Text style={styles.sheetStatValue}>{cf.rank || 'unrated'}</Text></Text>
          </View>
          ...
          <View style={styles.sheetPlatform}>
            ...
            <Text style={styles.sheetStat}>Stars: <Text style={styles.sheetStatValue}>{gh.totalStars || 0}</Text></Text>
            <Text style={styles.sheetStat}>Followers: <Text style={styles.sheetStatValue}>{gh.followers || 0}</Text></Text>
          </View>
        </View>
    ...
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      ...
      >
        <View style={styles.modalCard}>
          ...
          <Text style={styles.modalSubtitle}>Link your profiles to appear on the leaderboard.</Text>
...
export default function LeaderboardScreen() {
  ...
  const { entries, status, error, myHandles, handlesStatus, totalCount } = useSelector(
    (s) => s.leaderboard
  );
  ...
  const sortedEntries = useMemo(() => {
    return [...entries].sort(
      (a, b) => getPlatformScore(b, activeFilter) - getPlatformScore(a, activeFilter)
  ...
  const handleSaveHandles = useCallback(
    async (form) => {
      ...
      if (form.codeforces !== undefined) payload.codeforces = form.codeforces || null;
  ...
  const lastSynced = entries[0]?.lastSyncedAt
    ? new Date(entries[0].lastSyncedAt).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })
    : null;
    ...
    <SafeAreaView style={styles.container}>
      ...
      {status === 'loading' && !refreshing ? (
        ...
        <View style={styles.centered}>
          ...
          <TouchableOpacity style={styles.retryBtn} onPress={() => dispatch(fetchLeaderboard())}>
        ...
        <View style={styles.centered}>
          ...
          <Text style={styles.emptySubtitle}>
            Students with coding handles will appear here once the data is synced.
          </Text>
        </View>
      ) : (
        <FlatList
          ...
          ListHeaderComponent={
            <>
              {/* Total count */}
              <Text style={styles.countLabel}>{totalCount} students ranked</Text>

              {/* Top 3 Podium */}
              {top3.length > 0 && (
                <View style={styles.podiumRow}>
                  {/* 2nd place (left) */}
                  {top3[1] && <PodiumCard entry={top3[1]} position={1} filterKey={activeFilter} />}
                  {/* 1st place (center, taller) */}
                  {top3[0] && (
                    <View style={styles.podiumCenter}>
                      <PodiumCard entry={top3[0]} position={0} filterKey={activeFilter} />
                    </View>
                  )}
                  {/* 3rd place (right) */}
                  {top3[2] && <PodiumCard entry={top3[2]} position={2} filterKey={activeFilter} />}
                </View>
              )}
          ...
          renderItem={({ item }) => (
            <LeaderboardRow entry={item} onPress={setSelectedEntry} filterKey={activeFilter} />
          )}
      ...
      <View style={styles.myHandlesCard}>
        <View style={styles.myHandlesInfo}>
          ...
          <View style={styles.myHandlesList}>
            {myHandles?.leetcode ? (
              <Text style={styles.myHandle}><Text style={{ color: '#FFA116' }}>LC</Text> {myHandles.leetcode}</Text>
            ) : null}
            {myHandles?.codeforces ? (
              <Text style={styles.myHandle}><Text style={{ color: '#5CC2F2' }}>CF</Text> {myHandles.codeforces}</Text>
            ) : null}
            {myHandles?.github ? (
              <Text style={styles.myHandle}><Text style={{ color: '#9CA3AF' }}>GH</Text> {myHandles.github}</Text>
            ) : null}
            {!myHandles?.leetcode && !myHandles?.codeforces && !myHandles?.github && (
...
const styles = StyleSheet.create({
  ...
  filterRowContent: { paddingHorizontal: 16, gap: 8, alignItems: 'center', paddingVertical: 8 },
  filterChip: {
  ...
  filterChipTextActive: { color: '#FFD700', fontWeight: '700' },

  // States
  centered: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 32 },
  loadingText: { color: '#6B7280', marginTop: 12, fontSize: 14 },
  emptyTitle: { color: '#D1D5DB', fontSize: 18, fontWeight: '700', marginTop: 16 },
  emptySubtitle: { color: '#6B7280', fontSize: 13, textAlign: 'center', marginTop: 8, lineHeight: 20 },
  retryBtn: {
    marginTop: 20,
  ...
  // Count label
  countLabel: { color: '#4B5563', fontSize: 12, textAlign: 'center', marginTop: 4, marginBottom: 12 },
  ...
  podiumName: { color: '#FFFFFF', fontSize: 12, fontWeight: '600', textAlign: 'center' },
  podiumDept: { color: '#6B7280', fontSize: 10, textAlign: 'center', marginTop: 2 },
  podiumScoreBadge: {
    marginTop: 8,
  ...
  divider: { height: 1, backgroundColor: '#1F2937', marginHorizontal: 16, marginBottom: 8 },
  ...
  rowScore: { color: '#FFD700', fontSize: 16, fontWeight: '700' },
  ...
  sheetPlatformHeader: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 8 },
  ...
  modalSubtitle: { color: '#6B7280', fontSize: 13, marginBottom: 20, lineHeight: 18 },
  modalField: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#0D1321',
  ...
  myHandlesTitle: { color: '#9CA3AF', fontSize: 11, fontWeight: '600', marginBottom: 4, letterSpacing: 0.8 },
