...
import { Calendar, Clock, MapPin, User, AlertTriangle } from 'lucide-react-native';
...
export default function TimetableScreen() {
  ...
  const { schedule, isLoading, error } = useSelector((state) => state.timetable);
  ...
  const getFilteredClasses = () => {
    return schedule.filter((item) => item.isRecurring && item.dayOfWeek === selectedDay);
  };
      ...
      <View style={styles.classCard}>
        ...
        <View style={styles.classDetails}>
          <View style={styles.classHeader}>
            <Text style={styles.classCode}>{item.subjectId?.code || 'SUB'}</Text>
            <View style={styles.typeBadge}>
              <Text style={styles.typeBadgeText}>{item.type.toUpperCase()}</Text>
            </View>
          </View>
          <Text style={styles.className}>{item.subjectId?.name || 'Subject'}</Text>
  ...
  const renderExamItem = ({ item }) => {
    return (
      <View style={[styles.classCard, styles.examCard]}>
        ...
        <View style={styles.classDetails}>
          <View style={styles.classHeader}>
            <Text style={[styles.classCode, { color: '#F59E0B' }]}>{item.subjectId?.code || 'SUB'}</Text>
            <View style={[styles.typeBadge, { backgroundColor: 'rgba(245, 158, 11, 0.15)' }]}>
              <Text style={[styles.typeBadgeText, { color: '#F59E0B' }]}>EXAM</Text>
            </View>
          </View>
          <Text style={styles.className}>{item.subjectId?.name || 'Subject'}</Text>
          ...
          <View style={styles.infoRowContainer}>
            ...
            <View style={styles.infoIconText}>
              <User size={14} color="#9CA3AF" />
              <Text style={styles.infoText}>Invigilator: {item.facultyId?.userId?.name || 'Staff'}</Text>
            </View>
          </View>
        </View>
      </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerSubtitle}>Weekly slots and upcoming exams</Text>
      </View>

      {/* Mode Selector (Classes vs Exams) */}
      <View style={styles.modeTabBar}>
        <TouchableOpacity
          style={[styles.modeTabButton, viewMode === 'classes' && styles.modeTabButtonActive]}
          onPress={() => setViewMode('classes')}
          activeOpacity={0.7}
        >
          <Text style={[styles.modeTabLabel, viewMode === 'classes' && styles.modeTabLabelActive]}>Classes</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.modeTabButton, viewMode === 'exams' && styles.modeTabButtonActive]}
          onPress={() => setViewMode('exams')}
          activeOpacity={0.7}
        >
          <Text style={[styles.modeTabLabel, viewMode === 'exams' && styles.modeTabLabelActive]}>Exams</Text>
        </TouchableOpacity>
      </View>
      ...
      {error ? (
        ...
        <FlatList
          data={viewMode === 'classes' ? getFilteredClasses() : getFilteredExams()}
